...
- **Phase B — 서비스 신청 (concierge_logs)**: 마이그레이션
  ...
  않음). 시크릿 쿠폰 열람도 `coupon_view`로 함께 기록. 이어서
  `request_type='chat'`(매 턴 종료 시 사용자·어시스턴트 메시지를 각 500자
  요약 저장, 이미지는 `[사진 첨부]`로만 표시)과 `tmap_send`(T맵 딥링크
  클릭 시 `/api/log-tmap`으로 fire-and-forget 기록)도 마저 연결해,
  `concierge_logs`의 4개 request_type 전부를 실제로 채우는 상태가 됐다.
- **AI 제공자 전환(Claude → OpenAI, 2026-07)**: 이미 보유한 OpenAI
  크레딧/구독을 활용하기 위해 `stayaphae-concierge`의 `/api/chat`을
  Claude API에서 OpenAI API(`gpt-4o`, Chat Completions, 스트리밍 +
  function calling)로 교체. 날씨·물때·시크릿 쿠폰·바베큐 신청 4개
  커스텀 도구는 동일하게 유지. Claude의 서버 도구였던 웹서치는
  Chat Completions에 대응 기능이 없어 제거하고, 축제 등 시기성 질문은
  검색 없이 문의 안내로 답하도록 페르소나를 수정했다(실사용 로그로
  웹서치 필요성이 확인되면 별도 검색 API 연동을 재검토). 두 제공자를
  동시에 유지하는 폴백 구조는 이 앱 규모에 비해 과한 유지보수 부담이라
  두지 않았다. 자세한 내용은 `stayaphae-concierge`의 README.md "AI
  제공자 전환" 절 참고.
