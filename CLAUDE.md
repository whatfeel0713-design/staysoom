...
- `src/app/guide/` — 게스트 가이드(압해 컨시어지 3a 단계). **예약 확정 고객 전용 비공개 링크** — 사이트 내 공개 링크 없음, noindex, sitemap 제외. 체크인 안내와 함께 QR/링크로만 공유한다(3b에서 예약 코드 인증 예정). 랜딩 컨시어지 섹션은 소개만 노출(`CONCIERGE_LINK = null` 유지). 콘텐츠는 파일 상단 데이터 배열만 수정하면 됨.
