...
- `src/app/page.tsx` — 메인 랜딩. 상단의 `EXPERIENCES` / `CONCIERGE_FEATURES` / `AMENITIES` 배열이 기본 콘텐츠 데이터. 스테이 섹션의 4개 공간 카드(거실/침실/특별한 공간/마당)는 `src/lib/spaces.ts`의 `SPACES`를 사용하며, 각 카드는 `/space/[slug]` 상세 갤러리로 연결된다. `/admin/content`의 `content_blocks`가 있으면 히어로 배경(banner)·스테이 섹션 카드(room)·영상 밴드(youtube)·후기 섹션(testimonial)은 DB 콘텐츠가 우선 렌더링되고(이 경우 카드는 상세 갤러리 링크 없이 단순 카드로 표시), 없거나 조회 실패 시 하드코딩 기본값으로 폴백한다. `CONCIERGE_LINK`에 URL을 넣으면 AI 컨시어지 섹션의 "준비 중" 안내가 링크 버튼으로 바뀐다.
- `src/lib/spaces.ts` + `src/app/space/[slug]/page.tsx` — 공간별 상세 갤러리("갤러리 워크"). 공간당 히어로 1장 + 갤러리 3장 구성. 사진은 전부 Unsplash 플레이스홀더 — 리모델링 후 실사진으로 교체 시 이 배열의 `heroImage`/`gallery`만 수정하면 됨.
